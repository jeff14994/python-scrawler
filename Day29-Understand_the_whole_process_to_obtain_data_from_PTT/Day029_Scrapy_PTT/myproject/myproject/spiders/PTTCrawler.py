...
class PttcrawlerSpider(scrapy.Spider):
    ...
    def start_requests(self):
        # 使用 yield，即非同步請求
        # callback 填入 self.parse(即第25行的 parse())
        yield scrapy.Request(url=self.start_urls, callback=self.parse, cookies=self.cookies)
    # 剖析資料，Requtest 的 callback function 即是 parse()
    def parse(self, response):
        # 取得列表中的清單主體
        soup = BeautifulSoup(response.text)
        # bbs-screen class 是整個文章的 class
    ...
    def parse_article(self, response):
        ...
        for v in main_content.stripped_strings:
            ...
            if v[0] not in [u'※', u'◆'] and v[:2] not in [u'--']:
                # 將filter 後的正常本文 append 起來
                filtered.append(v)

        # 定義一些特殊符號與全形符號的過濾器
        expr = re.compile(u'[^一-龥。；，：“”（）、？《》\s\w:/-_.?~%()]')
        for i in range(len(filtered)):
            # re.sub 的 Resources: https://tinyl.io/3E30
            # re.sub() 最重要的必備三個參數 - 1.pattern(符合的條件) 2. repl(即將取代的字串) 3. string(被過濾且準備被取代的字串) ，對應到下面即 pattern -> expr, repl -> '', string -> filtered[i]
            # 下面的意思即為，依序列出要被過濾的字串(filtered[i])，若符合條件(expr)，則將其置換成''(即空白)
        ...
        # Path().stem Resources: https://tinyl.io/3E32 (search .stem)
